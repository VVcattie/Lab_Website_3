...
app.get('/team_stats', function(req,res) {
	var readData = 'SELECT*FROM football_games;';
	var countWins = 'SELECT count(*) FROM football_games WHERE home_score>visitor_score;';
	var countLoses = 'SELECT count(*) FROM football_games WHERE home_score<visitor_score;';
	db.task('get-everything', task => {
		return task.batch([
			task.any(readData),
			task.any(countWins),
	...
	.then(info => {
		res.render('pages/team_stats',{
			data: info[0],
			winNum: info[1][0].count,
			loseNum: info[2][0].count
		})
	})
	.catch(error => {
		console.log(error);
...
app.get('/player_info', function(req, res) {
	var readID = 'SELECT id, name FROM football_players;';
	db.any(readID)
    .then(function (rows) {
...
app.get('/player_info/select_player', function(req, res) {
	var readID = 'SELECT id, player_name FROM football_players;';
	var player_choice = req.query.player_choice;
	var readPlayer = 'SELECT*FROM football_players WHERE id='+player_choice+';';
	var gamePlayed = 'SELECT count(*) FROM football_games;';
  db.task('get-everything', task => {
    return task.batch([
      task.any(readID),
      task.any(readPlayer),
